package com.liujl.quick.module.infra.service.codegen.inner;
...
import com.liujl.quick.framework.apilog.core.annotation.ApiAccessLog;
import com.liujl.quick.framework.apilog.core.enums.OperateTypeEnum;
import com.liujl.quick.framework.common.exception.util.ServiceExceptionUtil;
...
import com.liujl.quick.framework.common.util.collection.CollectionUtils;
import com.liujl.quick.framework.common.util.date.DateUtils;
import com.liujl.quick.framework.common.util.date.LocalDateTimeUtils;
import com.liujl.quick.framework.common.util.object.BeanUtils;
import com.liujl.quick.framework.common.util.object.ObjectUtils;
import com.liujl.quick.framework.common.util.string.StrUtils;
import com.liujl.quick.framework.excel.core.annotations.DictFormat;
import com.liujl.quick.framework.excel.core.convert.DictConvert;
import com.liujl.quick.framework.excel.core.util.ExcelUtils;
import com.liujl.quick.framework.mybatis.core.dataobject.BaseDO;
...
import com.liujl.quick.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.liujl.quick.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.liujl.quick.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.liujl.quick.module.infra.enums.codegen.CodegenSceneEnum;
import com.liujl.quick.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.liujl.quick.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.liujl.quick.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 quick-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 quick-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.liujl.quick.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/quick-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/quick-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "quick-module-${table.moduleName}/" + // 顶级模块
                "quick-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "quick-module-${table.moduleName}/" + // 顶级模块
                "quick-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "quick-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "quick-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "quick-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "quick-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
